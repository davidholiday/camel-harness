...
* __com.charter.camel.service...__
...
* __com.charter.camel.service.test...__
...
* __create environment variables meant for use only when the app is deployed?__

    * In our environment, we deploy using Docker/Kubernetes. Inside the same pod this app will exist is another project called a __config-sidecar__. This project will ensure any properties it has will be be placed in the pod's filesystem at ```./config-volume```. Shared properties from ******* will be put in the filesystem at directory ```./shared-config-volume```. The property ```additional.properties.source.url.local``` in ```config.properties``` directs Archaius to look for properties at those locations and is how the app is made aware of them. 

* __override app behavior based on runtime context?__

    * this is what the AppContextLifecycle Harness is for. See ```com.charter.camel.service.context.HarnessedAppContextLifecycle``` for example. Note that this will completely override the default behaviour of the base AppContextLifecycle object. 
